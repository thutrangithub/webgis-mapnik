...
app.get('/api/generate-token', async (req, res) => {
  try {
    ...
    if (!username || !password) {
      return res.status(401).send('Vui lòng nhập đầy đủ tên người dùng và mật khẩu')
    }
    const db = new connectDb()
    const mtk = new manageToken()
    const uuid = uuidv4()
    const response = await db.LoginCheck(username, password)
    if (response == 0) { res.status(401).send('Tên người dùng không tồn tại') }
    else if (response == 2) { res.status(401).send('Sai mật khẩu') }
    else if (response == 1) {
      const token = await mtk.GenerateToken(uuid)
      res.send(token)
    }
  } catch (err) {
    console.log(err?.stack);
